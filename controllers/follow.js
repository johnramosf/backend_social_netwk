...
//Método para guardar un follow(seguimiento a a otro usuario)
export const saveFollow = async (req, res) => {
  try {
    // Obtener los datos desde el body
    const { followed_user } = req.body
    //Obtener el id del usuario auténticado
    const identity = req.user  

    //verificar si el identity contiene al usuario autenticado
    ...
    //Guardar el follow en la base de datos
    const followStored = await newFollow.save() 

    // Verificar si se guardo correctamente el la BD
    if(!followStored){
    ...
    //combinamos los datos del follow y followedUSer
